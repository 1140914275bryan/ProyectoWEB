...
$("#btnBuscar").click(function () {
    const filtro = $("#cboBuscarPor").val();

    if (filtro == "fecha") {
        if ($("#txtFechaInicio").val().trim() === "" || $("#txtFechaFin").val().trim() === "") {
            toastr.warning("", "Debe ingresar una fecha de inicio y de fin");
            return;
        }
    } else {
    ...
    fetch(`/Venta/Historial?numeroVenta=${numeroVenta}&fechaInicio=${fechaInicio}&fechaFin=${fechaFin}`)
        ...
        .then(responseJson => {
            ...
            if (responseJson.length > 0) {
                ...
                toastr.info("", "No se encontraron resultados con los filtros aplicados");
            }
        })
        .catch(error => {
            console.error("Error al obtener el historial:", error);
            toastr.error("", "Ocurrió un error al buscar las ventas");
            $(".card-body").find("div.row").LoadingOverlay("hide");
        });
});
